...
function update_coefficients_rhs_adjoint!(solver::PNImplicitMidpointSolver, problem::DiscretePNSystem, i, Δϵ)
    ...
    for e in 1:length(solver.a)
        si = problem.s[e, i]
        τi = problem.τ[e, i]
        solver.a[e] = -si + 0.5 * Δϵ * τi
        for sc in 1:length(solver.c[e])
            σi = problem.σ[e, sc, i]
            solver.c[e][sc] = -0.5 * Δϵ * σi
        end
    end
    b1 = 0.5*Δϵ
    b2 = -0.5*Δϵ
...
struct PNDLRFullImplicitMidpointSolver{T, V<:AbstractVector{T}, TPP, Tsolv} <: PNImplicitMidpointSolver{T}
    proj_problem::TPP
    a::Vector{T}
    c::Vector{Vector{T}}
    Mbuf::V
