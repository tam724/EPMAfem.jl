...
direction_model = EPMAfem.SphericalHarmonicsModels.EEEOSphericalHarmonicsModel(21, 1)
...
taylor_2nd_fd_03 = zeros(size(hs)..., Nδs)
...
for n in 1:Nδs
    ...
    for (i, h) in enumerate(hs)
        ...
        taylor_2nd_fd_03[i, n] = abs(Cδρs - C - h*dot(grad_fd_03, δρs))
    end
end
...
scatter!(hs, sum(taylor_2nd_fd_03; dims=2)/Nδs, xaxis=:log, yaxis=:log, label="2nd rem. (FD, Δ=0.001)")
