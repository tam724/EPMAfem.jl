...
grid_gen_2D((-1.5, 1.5, -1.5, 1.5); min_res=0.08, max_res=0.08)
space_model = EPMAfem.SpaceModels.GridapSpaceModel(DiscreteModelFromFile("/tmp/tmp_msh.msh"); plus=(name=lagrangian, order=1, conformity=:H1), minus=(name=lagrangian, order=0, conformity=:L2))

space_model = EPMAfem.SpaceModels.GridapSpaceModel(CartesianDiscreteModel((-1.5, 1.5, -1.5, 1.5), (40, 40)); plus=(name=lagrangian, order=1, conformity=:H1), minus=(name=lagrangian, order=0, conformity=:L2))
N = 21
direction_model = EPMAfem.SphericalHarmonicsModels.EOSphericalHarmonicsModel(N, 2, :EO)
...
# system = EPMAfem.implicit_midpoint_dlr5(problem; tolerance=1e-2);
...
@gif for (ϵ, ψ) in sol
    ...
    f = EPMAfem.SpaceModels.interpolable((p=collect(ψp*bΩp), m=collect(ψm*bΩm)), EPMAfem.space_model(sol.system.problem.model))
    p = heatmap(-1.4:0.01:1.4, -1.4:0.01:1.4, (x, y) -> f(VectorValue(x, y)), aspect_ratio=:equal)
end
...
model = CartesianDiscreteModel((-1.5, 1.5, -1.5, 1.5), (10, 10))
model = DiscreteModelFromFile("/tmp/tmp_msh.msh")
V = TestFESpace(model, ReferenceFE(lagrangian, Float64, 1), conformity=:H1)
V1 = TestFESpace(model, ReferenceFE(lagrangian, Float64, 1), conformity=:L2)

function project(f, model, V)
    Ω = Triangulation(model)
    dx = Measure(Ω, 5)
    A = assemble_matrix((u, v) -> ∫(u*v)dx, TrialFESpace(V), V)
    b = assemble_vector(v -> ∫(f*v)dx, V)
    return FEFunction(V, A\b)
end

f = project(x -> sin(2*x[1])*cos(2*x[2]), model, V)
heatmap(-1.5:0.01:1.5, -1.5:0.01:1.5, (x, y) -> f(VectorValue(x, y)))

f_grad = project(dot(∇(f), VectorValue(0.0, 1.0)), model, V1)
heatmap(-1.5:0.01:1.5, -1.5:0.01:1.5, (x, y) -> f_grad(VectorValue(x, y)))
