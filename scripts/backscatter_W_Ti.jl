...
pnsystem = EPMAfem.schurimplicitmidpointsystem(upd_pnproblem.problem)

outflux = EPMAfem.discretize_outflux(model, EPMAfem.cuda())

excitation = EPMAfem.pn_excitation([(x=dimless(x_, eq.dim_basis), y=0.0) for x_ in range(-500u"nm", 500u"nm", length=300)], [dimless(15.0u"keV", eq.dim_basis)], [VectorValue(-1.0, 0.0, 0.0)], beam_position_σ=dimless(50u"nm", eq.dim_basis), beam_energy_σ=dimless(0.5u"keV", eq.dim_basis))
