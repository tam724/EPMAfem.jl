...
mat = [n"W", n"Ti"]
ϵ_range = range(50u"eV", 17.0u"keV", length=50)
eq = NExt.epma_equations(mat, ϵ_range, 21)
model = NExt.epma_model(eq, (-1500.0u"nm", 0.0u"nm", -1500u"nm", 1500u"nm"), (200, 320), 17)
...
excitation = EPMAfem.pn_excitation([(x=dimless(x_, eq.dim_basis), y=0.0) for x_ in range(-700u"nm", 700u"nm", length=300)], [dimless(15.0u"keV", eq.dim_basis)], [VectorValue(-1.0, 0.0, 0.0)], beam_position_σ=dimless(50u"nm", eq.dim_basis), beam_energy_σ=dimless(0.5u"keV", eq.dim_basis), beam_direction_κ=50.0)
...
function mass_concentrations(e, x)
    if x[2] < 0.0
        ...
        return e == 1 ? 0.0 : dimless(mat[2].density, eq.dim_basis)
    end
end
...
inflx = EPMAfem.compute_influx(excitation, model)
#plot(inflx[1, :, 1])

@time res = (outflux * pnsystem) * discrete_rhs

outflx = 2.0*res .+ inflx
...
plot(dimful(getindex.(excitation.beam_positions, :x), u"nm", eq.dim_basis), -outflx[1, :, 1] ./ inflx[1, :, 1])

using Serialization

serialize("results_wti.jls", (dimful(getindex.(excitation.beam_positions, :x), u"nm", eq.dim_basis), -outflx[1, :, 1] ./ inflx[1, :, 1]))
