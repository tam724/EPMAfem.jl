...
space_model = EPMAfem.SpaceModels.GridapSpaceModel(CartesianDiscreteModel((-1.5, 1.5, -1.5, 1.5), (30, 30)), plus=(name=lagrangian, order=1, conformity=:H1), minus=(name=lagrangian, order=0, conformity=:L2))
...
source = EPMAfem.Rank1DiscretePNVector(false, model, arch, zeros(EPMAfem.n_basis(model).nϵ), (p=zeros(EPMAfem.n_basis(model).nx.p), m=zeros(EPMAfem.n_basis(model).nx.m)), (p=zeros(EPMAfem.n_basis(model).nΩ.p), m=zeros(EPMAfem.n_basis(model).nΩ.m)))
...
σ = 0.03
using EPMAfem.HCubature
init_x(x) = 1/(σ*sqrt(2π))*exp(-1/2*((x[1])^2 + (x[2])^2)/σ^2)
...
Ωp_zm, Ωm_zm = EPMAfem.SphericalHarmonicsModels.eval_basis(EPMAfem.direction_model(model), Ω->EPMAfem.Dimensions.Ωz(Ω)/(4π)) |> EPMAfem.architecture(problem)
Ωp_xm, Ωm_xm = EPMAfem.SphericalHarmonicsModels.eval_basis(EPMAfem.direction_model(model), Ω->EPMAfem.Dimensions.Ωx(Ω)/(4π)) |> EPMAfem.architecture(problem)

@show model.space_mdl.discrete_model.grid.node_coords .|> Base.Fix2(getindex, 1)
regions = [((-1.5, 1.5), (-1.5, 1.5)), ((-0.2, 0.4), (-0.2, 0.4)), ((0.2, 0.5), (0.2, 0.5))]
# fix the regions:
...
in_interval(x::Real, (l, r)) = x >= l && x <= r
in_interval(x, r) = all(in_interval(x_, r_) for (x_, r_) in zip(x, r))

nb = EPMAfem.n_basis(model)
for region in regions
    xp, xm = EPMAfem.SpaceModels.eval_basis(EPMAfem.space_model(model), x -> in_interval(x, region))
    push!(x_r, (p=xp, m=xm) |> EPMAfem.architecture(problem))

    # xp_tilde = collect(Mp) \ xp
    xm_tilde = collect(Mm) \ xm
    ...
    xp_zb = unlazy(problem.space_discretization.∇pm[1], ws_size->EPMAfem.allocate_vec(arch, ws_size)) * xm_tilde
    xp_xb = unlazy(problem.space_discretization.∇pm[2], ws_size->EPMAfem.allocate_vec(arch, ws_size)) * xm_tilde
    xm_b = zeros(nb.nx.m) |> arch
    push!(x_b_r, (z=(p=xp_zb, m=xm_b), x=(p=xp_xb, m=xm_b)))
end
...
@gif for (i, (ϵ, ψ)) in enumerate(sol)
    ...
    func = EPMAfem.SpaceModels.interpolable((p=ψp*Ωp |> collect, m=ψm*Ωm |> collect), EPMAfem.space_model(model))
    heatmap(-1.5:0.01:1.5, -1.5:0.01:1.5, (z, x) -> func(VectorValue(x, z)))
    mass[i] = dot(x_r[1].p, ψp*Ωp) + dot(x_r[1].m, ψm*Ωm)
    net_flux[i] = dot(x_b_r[1].z.p, ψp*Ωp_zm) + dot(x_b_r[1].z.m, ψm*Ωm_zm) + dot(x_b_r[1].x.p, ψp*Ωp_xm) + dot(x_b_r[1].x.m, ψm*Ωm_xm)
    for j in 1:length(regions)
        contour!(-1.5:0.01:1.5, -1.5:0.01:1.5, (z, x) -> in_interval(VectorValue(x, z), regions[j]), cmap=nothing, legend=nothing, color=:gray)
        local_mass[i, j] = dot(x_r[j].p, ψp*Ωp) + dot(x_r[j].m, ψm*Ωm)
        local_net_flux[i, j] = dot(x_b_r[j].z.p, ψp*Ωp_zm) + dot(x_b_r[j].z.m, ψm*Ωm_zm) + dot(x_b_r[j].x.p, ψp*Ωp_xm) + dot(x_b_r[j].x.m, ψm*Ωm_xm)
...
begin
    ...
    plot!(mass .+ cumtrapz(net_flux, EPMAfem.energy_model(model).step), label="mass + outflux", ls=:dot, color=1, legend=:right)
