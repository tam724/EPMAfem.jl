...
xp_b = vec(problem.space_discretization.∇pm[1] * (Mm \ xm))
Ωp_b = Ωp_m 
...
system_lr10_aug = EPMAfem.implicit_midpoint_dlr5(problem, solver=LinearAlgebra.:\, max_ranks=(p=5, m=5), basis_augmentation=:mass);
system_lr14_aug = EPMAfem.implicit_midpoint_dlr5(problem, solver=LinearAlgebra.:\, max_ranks=(p=7, m=7), basis_augmentation=:mass);
system_lr18_aug = EPMAfem.implicit_midpoint_dlr5(problem, solver=LinearAlgebra.:\, max_ranks=(p=9, m=9), basis_augmentation=:mass);
...
begin
    ...
    plot!(t, abs.(masses[:, 3] .+ cumtrapz(outflux[:, 3], step(energy_model)) .- M0), color=2, ls=:dash, label=nothing)
    plot!(t, abs.(masses[:, 4] .+ cumtrapz(outflux[:, 4], step(energy_model)) .- M0), color=3, ls=:dash, label=nothing)
    plot!(t, abs.(masses[:, 5] .+ cumtrapz(outflux[:, 5], step(energy_model)) .- M0), color=4, ls=:dash, label=nothing)
