...
let
    ...
    averaged_sol = probe((EPMAfem.tangent(updatable_pnproblem, λ, ρs)[1, 2465]) * discrete_system)
    # averaged_sol = probe(((EPMAfem.tangent(updatable_pnproblem, λ, ρs)[1, 2465]) + EPMAfem.tangent(discrete_ext[1], ρs)[1, 2465]) * discrete_system)
    func = EPMAfem.SpaceModels.uncached_interpolable(averaged_sol, space_model)
    Plots.contourf(range(-1, 0, 40), range(-0.8, 0.8, 120), func, swapxy=true, aspect_ratio=:equal, linewidth=0, cmap=reverse(cgrad(:roma)))
    Plots.xlabel!(L"x")
    Plots.ylabel!(L"z")
