...
function ReshapeableMatrix{T}(A) where T
    if A isa AbstractPNMatrix error("ReshapeableMatrix needs a `base` Matrix") end
    return ReshapeableMatrix{T}(A, Ref(size(A, 1)), Ref(size(A, 2)))
end
...
max_size(R::ReshapeableMatrix) = size(R.A)
...
function get_view(R::ReshapeableMatrix)
    return reshape(@view(R.A[1:R.nA[]*R.mA[]]), (R.nA[], R.mA[]))
end

function reshape!(R::ReshapeableMatrix, (nA, mA))
    @assert size(R.A, 1) >= nA && size(R.A, 2) >= mA
    R.nA[] = nA
    R.mA[] = mA
end
